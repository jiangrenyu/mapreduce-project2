...
import com.bonc.mr.comm.MergeTextOutFormate;
import com.bonc.mr.comm.MergeCombineTextInputFormat;
import com.bonc.mr.comm.NewTextOutFormate;
 ...
 * JDK 1.7
 */
public class CombineTimeJudgeMapReaduce1 extends Configured implements Tool {
    private static final Log LOG = LogFactory.getLog(CombineTimeJudgeMapReaduce1.class);
    private static final String PROV_ID = "provIdNameOrCode";

    public static void main(String[] args) throws Exception {
        ToolRunner.run(new Configuration(), new CombineTimeJudgeMapReaduce1(), args);
    }
    ...
    public int run(String[] arg0) {
        try {
            ...
            Job job = Job.getInstance(conf, "Merge MapReduce");
            job.setJarByClass(CombineTimeJudgeMapReaduce1.class);
            //设置输入
            LOG.info("输入规则路径:" + argPasser.outpoutTmpDir);
            FileInputFormat.setInputPaths(job, argPasser.outpoutTmpDir);
            FileInputFormat.setInputDirRecursive(job, true);
            job.setInputFormatClass(MergeCombineTextInputFormat.class);
            ...
            //设置输出
            LOG.info("输出路径:" + argPasser.outputDir);
            FileOutputFormat.setOutputPath(job, new Path(argPasser.outputDir));
            job.setOutputFormatClass(MergeTextOutFormate.class);
    ...
    private static class ArgPasser {
        ...
        public void confPasser() throws IOException {
            ...
            if (parser.get("o") != null) {
                this.outputDir = parser.get("o")+separator;
